...
impl<'a, E: ExtensionField> ClassicSumCheckRoundMessage<E> for Coefficients<'a, E> {
    type Auxiliary = ();
...
impl<'a, E: ExtensionField> ClassicSumCheckProver<E> for CoefficientsProver<'a, E> {
    type RoundMessage = Coefficients<'a, E>;
    ...
    fn karatsuba<const LAZY: bool>(
        ...
        match (lhs, rhs) {
            ...
            | (
                ...
                if poly_len == 1 {
                    let mul = state.size();
                    let mul_field = E::from_canonical_u64(mul as u64);
                    ...
                    let multiple_field = E::from_canonical_u64(multiple as u64);
