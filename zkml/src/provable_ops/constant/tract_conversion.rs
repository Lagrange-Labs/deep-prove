...
impl TryFrom<Const> for Constant<f32> {
    type Error = ProvableOpError;

    fn try_from(tract_op: Const) -> Result<Constant<f32>, ProvableOpError> {
        ...
        let tensor: DeepTensor<f32> = tract_tensor.as_ref().try_into()?;
