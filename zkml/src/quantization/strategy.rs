...
use crate::{Element, Tensor, layers::Layer, model::Model, quantization};
            ...
            .map(|(id,layer)| {
                match layer {
                    Layer::Dense(dense) => {
                        ...
                        info!("InferenceObserver: DENSE {} layer",step_idx);
                    ...
                    Layer::Convolution(conv) => {
                        ...
                        md.set_layers_scaling(step_idx, output_scaling);
                        {
                            ...
                            info!("InferenceObserver: CONV {} layer f32 output quantized: min {},max {}", step_idx, min_quantized, max_quantized);
                            ...
                            info!("InferenceObserver: CONV {} layer f32 weight quantized: min {},max {}", step_idx, min_weight, max_weight);
                            info!("InferenceObserver: CONV {} layer f32 bias quantized: min {},max {}", step_idx, min_bias, max_bias);
                        }
                    ...
                    //}
                    a => {
                        info!("InferenceObserver: ANY {} layer {}",step_idx,a.describe());
                        step_idx += 1;
            ...
            .flat_map(|(id,l)| {
                ...
                match l {
                    Layer::Dense(d) => {
                        ...
                        info!("Scaling: AbsoluteMax: DENSE max_weight {:?}, max_output: {:?} - adding requant", max_weight, 1.0);
