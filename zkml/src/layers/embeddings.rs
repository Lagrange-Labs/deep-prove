...
use crate::{padding::PaddingMode, parser::gguf::FileTensorLoader, tensor::Number, Element, Tensor};
...
impl<N: Number> Embeddings<N> {
    pub fn forward(&self, x: &Tensor<usize>) -> anyhow::Result<Tensor<N>> {
        assert!(x.get_shape().len() == 1, "embeddings only support 1d tensors");
        ...
        let emb = x.get_data().par_iter().flat_map(|&idx| {
            assert!(idx < vocab_size,"idx {} out of bounds for vocab size {}", idx, vocab_size);
...
mod tests {
    ...
    fn generate_unique_random_indices(seq_len: usize, vocab_size: usize) -> Vec<usize> {
        let mut ctr = 0;
        while ctr < 10 {
            let d = (0..seq_len).map(|_| thread_rng().gen_range(0..vocab_size)).collect::<Vec<_>>();
            let mut dd = d.clone();
            dd.sort();
            dd.dedup();
            if dd.len() == seq_len {
    ...
    fn test_embeddings() -> anyhow::Result<()> {
        ...
        let emb_vector = |idx:usize| -> Vec<Element> {
            (0..emb_size).map(|j| Element::from((10000 * idx + j) as Element)).collect()
        };
