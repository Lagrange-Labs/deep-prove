        ...
        for i in 0..num_columns {
            for j in 0..num_columns {
                for h in 0..num_heads {
                    let x_i = to_be_bits(Element::from(i as u64), NUM_BITS).unwrap();
                    let y_i = to_be_bits(Element::from(j as u64), NUM_BITS).unwrap();
                    let h_i = to_be_bits(Element::from(h as u64), NUM_HEADS_BITS).unwrap();
