...
impl Evaluate<f32> for LayerNorm<f32> {
    fn evaluate<E: ff_ext::ExtensionField>(
        ...
        let data:Vec<f32> = output.to_data().into_vec().map_err(|e| ProvableOpError::ConversionError("failed to convert to f32".to_string()))?;
...
impl Evaluate<Element> for LayerNorm<Element> {
    ...
    ) -> anyhow::Result<super::provable::LayerOut<Element, E>, super::provable::ProvableOpError> {
        unimplemented!()
    }
}
